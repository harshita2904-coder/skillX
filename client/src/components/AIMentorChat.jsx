...
const AIMentorChat = ({ isOpen, onClose }) => {
  ...
  const loadWelcomeMessage = async () => {
    try {
      ...
      const response = await fetch(`https://skillx-i9on.onrender.com/dialogflow/welcome/${user._id}`, {
        headers: {
  ...
  const sendMessage = async (messageText) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://skillx-i9on.onrender.com/dialogflow/message', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
